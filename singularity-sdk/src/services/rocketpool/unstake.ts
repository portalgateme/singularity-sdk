...
import { DarkpoolError, Relayer, Token, UnstakeRocketPoolRelayerRequest } from '../../entities';
...
class UnStakeRocketPoolContext extends BaseRelayerContext {
...
export class UnStakeNoteRocketPoolService extends BaseRelayerService<
  UnStakeRocketPoolContext,
  UnstakeRocketPoolRelayerRequest,
  SingleNoteResult
> {
  ...
  public async prepare(
    ...
    outAsset: Token,
    signature: string
  ): Promise<{ context: UnStakeRocketPoolContext; outPartialNotes: PartialNote[] }> {
    const outNotePartial = await createPartialNote(outAsset.address, signature);

    const context = new UnStakeRocketPoolContext(this._darkPool.getRelayer(), signature);
    context.inNote = inNote;
    context.outNotePartial = outNotePartial;
    return { context, outPartialNotes: [outNotePartial] };
  }

  public async generateProof(context: UnStakeRocketPoolContext): Promise<void> {
  ...
  public async getRelayerRequest(context: UnStakeRocketPoolContext): Promise<UnstakeRocketPoolRelayerRequest> {
    if (
      !context ||
      !context.inNote ||
    ...
    const relayerRequest: UnstakeRocketPoolRelayerRequest = {
  ...
  public getRelayerPath(): string {
    return relayerPathConfig[Action.ROCKET_POOL_UNSTAKE];
  }

  public async postExecute(context: UnStakeRocketPoolContext): Promise<SingleNoteResult> {
    ...
    const event = await getOutEvent(context.tx, RocketPoolStakeAssetManagerAbi.abi, 'RocketWithdrawal', this._darkPool);
