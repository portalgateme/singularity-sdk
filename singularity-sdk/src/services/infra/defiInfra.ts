...
export class DefiInfraService extends BaseRelayerService<DefiInfraContext, DefiInfraRelayerRequest, MultiNotesResult> {
  ...
  public async prepare(
    request: DefiInfraRequest,
    signature: string
  ): Promise<{ context: DefiInfraContext; outPartialNotes: PartialNote[] }> {
  ...
  public async generateProof(context: DefiInfraContext): Promise<void> {
    ...
    const paths = await multiGetMerklePathAndRoot(
      inNotes.filter(item => item !== null).map(item => item?.note) as bigint[],
      this._darkPool
    );
