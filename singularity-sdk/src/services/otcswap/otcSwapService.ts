...
export class OTCSwapService extends BaseContractService<OTCSwapContext> {
  ...
  private checkFullSwapSecret(partialSwapMessage: OTCSwapFullMessage, order: Order): boolean {
    ...
    if (
      !isAddressEquals(order.makerAsset, partialSwapMessage.makerNote.asset) ||
    ...
    if (
      order.makerAmount != partialSwapMessage.makerNote.amount ||
  ...
  public async prepare(
    order: Order,
    fullSwapMessageString: string,
    signedMessage: string
  ): Promise<{ context: OTCSwapContext; outNotes: Note[] }> {
    const fullSwapMessage = fullSwapSecretFromString(this._darkPool.chainId, fullSwapMessageString);
  ...
  public async generateProof(context: OTCSwapContext): Promise<void> {
    ...
    const merklePathes = await multiGetMerklePathAndRoot(
      [context.swapMessage.makerNote.note, context.swapMessage.takerNote.note],
      this._darkPool
    );
