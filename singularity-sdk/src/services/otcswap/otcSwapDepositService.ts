...
function partialSwapSecretFromString(chainId: number, messageString: string): OTCSwapMakerMessage {
...
export class OTCSwapDepositService {
  ...
  private async generateMakerSwapMessage(
    chainId: number,
    order: Order,
    note: Note,
  ...
  public async prepareMakerAsset(
    order: Order,
    walletAddress: string,
    signedMessage: string
  ): Promise<{ partialSwapSecret: string; context: OTCSwapMakerDepositContext }> {
    const depositService = new DepositService(this._darkPool);
    const {
      context: depositContext,
      outNotes: [note]
    } = await depositService.prepare(order.makerAsset, order.makerAmount, walletAddress, signedMessage);
  ...
  private async generateTakerSwapMessage(
    order: Order,
    partialSwapMessage: OTCSwapMakerMessage,
    takerNote: Note,
    signedMessage: string
  ): Promise<string> {
  ...
  public async prepareTakerAsset(
    order: Order,
    partialSwapSecret: string,
    walletAddress: string,
    signedMessage: string
  ): Promise<{ finalSwapSecret: string; context: OTCSwapTakerDepositContext }> {
    ...
    const {
      context: depositContext,
      outNotes: [note]
    } = await depositService.prepare(order.takerAsset, order.takerAmount, walletAddress, signedMessage);
