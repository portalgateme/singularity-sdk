import {
  darkPoolCancelOrderProofResult,
  darkPoolMakerCreateOrderProofResult,
  generateDarkPoolCancelOrderProof,
  Note
...
export class CancelOrderService extends BaseContractService<CancelOrderContext> {
  ...
  public async prepare(
    outgoingNote: Note,
    signature: string
  ): Promise<{ context: CancelOrderContext; outNotes: Note[] }> {
  ...
  public async execute(context: CancelOrderContext): Promise<string> {
    ...
    const tx = await contract.cancelOrder(context.merkleRoot, context.proof.outNullifier, context.proof.proof.proof);
    return tx.hash;
  }
}
