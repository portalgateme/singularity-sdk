import {
  ...
  DarkPoolTakerSwapProofResult,
  Note,
  generateDarkPoolTakerSwapProof
...
export class TakerSwapService extends BaseContractService<TakerSwapContext> {
  ...
  public async prepare(
    aliceSwapMessage: DarkPoolTakerSwapMessage,
    bobSwapMessage: DarkPoolTakerSwapMessage
  ): Promise<{ context: TakerSwapContext; outNotes: Note[] }> {
  ...
  public async generateProof(context: TakerSwapContext): Promise<void> {
    ...
    const merklePathes = await multiGetMerklePathAndRoot(
      [context.alceSwapMessage.outNote.note, context.bobSwapMessage.outNote.note],
      this._darkPool
    );
  ...
  public async execute(context: TakerSwapContext): Promise<string> {
    ...
    const tx = await contract.takerSwap(context.proof.proof.proof, [
      context.merkleRoot,
