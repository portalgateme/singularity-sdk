import {
  NftNote,
  PartialNote,
  UniswapRemoveLiquidProofResult,
  createPartialNote,
  generateUniswapRemoveLiquidProof,
...
export class UniswapRemoveLiquidityService extends BaseRelayerService<
  UniswapRemoveLiquidityContext,
  UniswapRemoveLiquidityRelayerRequest,
  MultiNotesResult
> {
  ...
  public async prepare(
    request: UniswapRemoveLiquidityRequest,
    signature: string
  ): Promise<{ context: UniswapRemoveLiquidityContext; outPartialNotes: PartialNote[] }> {
  ...
  public async generateProof(context: UniswapRemoveLiquidityContext): Promise<void> {
    if (!context || !context.request || !context.outPartialNote1 || !context.outPartialNote2 || !context.signature) {
  ...
  protected async getRelayerRequest(
    context: UniswapRemoveLiquidityContext
  ): Promise<UniswapRemoveLiquidityRelayerRequest> {
    if (
      !context ||
      !context.request ||
  ...
  private async getOutAmounts(tx: string) {
    ...
    if (receipt && receipt.logs.length > 0) {
      const log = receipt.logs.find(log => log.topics[0] === 'UniswapRemoveLiquidity');
      if (log) {
