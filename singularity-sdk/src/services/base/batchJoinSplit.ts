import {
  BatchJoinSplitProofResult,
  Note,
  createNote,
  generateBatchJoinSplitProof,
  EMPTY_NOTE
...
export class BatchJoinSplitService extends BaseContractService<BatchJoinSplitContext> {
  ...
  public async prepare(
    ...
    outAmount1: bigint,
    signature: string
  ): Promise<{ context: BatchJoinSplitContext; outNotes: Note[] }> {
  ...
  public async generateProof(context: BatchJoinSplitContext): Promise<void> {
    ...
    const merklePathes = await multiGetMerklePathAndRoot(
      context.inNotes.map(note => note.note),
      this._darkPool
    );
  ...
  public async execute(context: BatchJoinSplitContext): Promise<string> {
    ...
    const contract = new ethers.Contract(
      this._darkPool.contracts.batchJoinSplitAssetManager,
      BatchJoinSplitAssetManagerAbi.abi,
    ...
    const tx = await contract.batchJoinSplit(
      ...
      [context.proof.outNoteFooter1, context.proof.outNoteFooter2],
